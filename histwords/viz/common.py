...
# plotting features, not used much any more
def plot_words_year(word1, words, fitted, sims,year):
    # TODO: remove this and just set the plot axes directly
    ...
    for i in xrange(len(words)):
        ...
        if ww == word1 or (isArray and ww in word1):
            ...
            color = 'black'
            # sizing = 15
        plt.text(pt[0], pt[1], word,color=color)
    return annotations
...
def words_diff_avg(words,year,end_year,seq_embeddings):
    # fiction_embeddings = SequentialEmbedding.load("eng-all", (year,end_year))
    # scores = dict()
    avg = 0
    for word in words:
...
def words_diff_min(words,year,end_year,seq_embeddings):
    # fiction_embeddings = SequentialEmbedding.load("eng-all", (year,end_year))
    # scores = dict()
    M = 3
    for word in words:
...
def get_word_rankings(words,year,end_year):
    fiction_embeddings = SequentialEmbedding.load("eng-all", (year,end_year))
    scores = dict()
    for word in words:
...
def words_diff_k(words,year,end_year,embeds,k):
    scores = 0
    if (type(words) == str):
...
def sort_words(words,year,end_year,embeds):
    # scores = dict()
    # ascending order
    words = set(ast.literal_eval(words))
